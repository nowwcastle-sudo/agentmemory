...
async function main() {
	...
	if (!sessionId) return;
	const assistantResponse = typeof data.last_assistant_message === "string" ? data.last_assistant_message.trim() : "";
	const cwd = hookCwd(data) || process.cwd();
	const delivery = defaultHookDelivery();
	if (assistantResponse) await delivery.deliver("/agentmemory/observe", {
		captureId: stableHookCaptureId(sessionId, "assistant", data.turn_id ?? assistantResponse),
